// CIni_Camera.cpp: Implementierung der Klasse CIni_Camera.
//
//////////////////////////////////////////////////////////////////////
...
CIni_Camera::CIni_Camera(CString Verz, CString Datei)
{
...
CIni_Camera::~CIni_Camera()
{
	if( !INI_Schreiben() )
		AfxMessageBox("Camera.ini konnte nicht neu geschrieben werden!");

}
CharReturn CIni_Camera::CheckChar( char zeichen )
{
	CharReturn ret;
...
void CIni_Camera::INI_LesenFileDialog()
{
	m_File.Abort();
...
bool CIni_Camera::INI_Lesen()
{
	...
	do{
		...
		if(m_CurrLine.Find(m_cstrPrincPtName) != -1){
			m_iPos = m_CurrLine.Find(m_cstrPrincPtName);
			Read_PrincipalPoint();
		}
...
void CIni_Camera::Read_PrincipalPoint(){
	
	m_File.ReadString(m_CurrLine); 
	if(m_CurrLine.GetLength()==0) return;
	m_StringArray.Add(m_CurrLine);
	CharReturn chartype;
	int end = m_CurrLine.Find(";")+1, second = m_CurrLine.Find(" "), i=0;
	m_iPos = 0; //m_iPos is set to beg. of data (" = [ ")
	
	do{
		if(m_iPos == second){
			m_Int_PrincPt[0] = atof(m_chrArray); i = 0; m_iPos += 1;
		}
...
void CIni_Camera::Read_DistortionMatlab(){
...
void CIni_Camera::Read_FocalLen(){
	
	m_File.ReadString(m_CurrLine); 
	if(m_CurrLine.GetLength()==0) return;
	m_StringArray.Add(m_CurrLine);
	CharReturn chartype;
	int end = m_CurrLine.Find(";")+1, i=0;
	m_iPos = 0; //m_iPos is set to beg. of data (" = [ ")
	
	do{
		if(m_iPos == end-1){
			m_Int_FocLen[0] = atof(m_chrArray); break;
		}
		chartype = CheckChar(m_CurrLine[m_iPos]);
		// Read current number if it is one
		if( chartype == NUMBER ){
...
void CIni_Camera::Read_CameraMatrix()
{
	CharReturn chartype;
...
void CIni_Camera::Read_Distortion()
{
	m_File.ReadString(m_CurrLine); 
	if(m_CurrLine.GetLength()==0) return;
	m_StringArray.Add(m_CurrLine);
...
void CIni_Camera::Read_ImageSize()
{
	m_File.ReadString(m_CurrLine); 
	if(m_CurrLine.GetLength()==0) return;
	m_StringArray.Add(m_CurrLine);
...
void CIni_Camera::INI_SchreibenFileDialog()
{
...
bool CIni_Camera::INI_Schreiben()
{
	CFileException error; CString str;
...
void CIni_Camera::Set_ImageSize(int x, int y)
{
	m_Int_ImageSize[0] = x;
	m_Int_ImageSize[1] = y;
}
...
void CIni_Camera::Set_CameraMat(double* aCameraMat)
{
	for(int i=0;i<9;i++)
...
void CIni_Camera::Set_DistortionVec(double* aDistortionVec)
{
...
void CIni_Camera::Read_CellSize()
{
	m_File.ReadString(m_CurrLine); m_StringArray.Add(m_CurrLine);
	CharReturn chartype;
	int end = m_CurrLine.Find(";"), second = m_CurrLine.Find(" "), i=0, j=0;
	m_iPos = 0; //m_iPos is set to beg. of data (" = [ ")
...
void CIni_Camera::Set_CellSize(double x, double y)
{
	m_Int_CellSize[0] = x;
	m_Int_CellSize[1] = y;
}

double CIni_Camera::Get_FocLen()
{
	return m_Int_FocLen[0];
}

double CIni_Camera::Get_CellSizeX()
{
	return m_Int_CellSize[0];
}

double CIni_Camera::Get_CellSizeY()
{
	return m_Int_CellSize[1];

}

int CIni_Camera::Get_PixNumX()
{
	return m_Int_ImageSize[0];
}

int CIni_Camera::Get_PixNumY()
{
	return m_Int_ImageSize[1];

}

void CIni_Camera::Set_FocLen(double aFocLen)
{
