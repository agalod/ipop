...
//DEL void CIIPM::push_back_ObjPt(double  aX, double  aY, double  aZ){
//DEL 	D3DVECTOR myPt;	myPt(0) = aX;	myPt(1) = aY;	myPt(2) = aZ; myPt(3) = 1;
//DEL 	fPts_Obj.push_back(myPt);}


//DEL void CIIPM::push_back_TestPt(double  aX, double  aY, double  aZ){
//DEL 	D3DVECTOR myPt;	myPt(0) = aX;	myPt(1) = aY;	myPt(2) = aZ; myPt(3) = 1;
//DEL 	fPts_Test.push_back(myPt);}


//DEL void CIIPM::push_back_FixedPt(double  aX, double  aY, double  aZ){
//DEL 	D3DVECTOR myPt;	myPt(0) = aX;	myPt(1) = aY;	myPt(2) = aZ; myPt(3) = 1;
...
int CIIPM::Calc_RegIIPM(){   
   //mlfSetErrorHandler(ErrorHandler);
	...
	double mean = 0;	int Num_Iteration=0;
...
//DEL double CIIPM::Calc_DistanceLinePoint( D3DVECTOR& aProjR, D3DVECTOR& aObjP){
//DEL 	double distance = Magnitude(CrossProduct(aObjP, aProjR)) / Magnitude(aProjR);
//DEL 	return distance;}
...
//; (CENTERED COORDINATES (CAMERA COORDINATE SYSTEM; 0,0 IS IN THE CENTER OF THE IMAGE )
	double x_pix = 0, y_pix = 0, x_mm = 0, y_mm = 0;
...
void CIIPM::Convert_MxArray( mxArray* aSourceArray, D3DMATRIX& aDestMatrix ){
	double value = 0;	mxArray* result = 0; aDestMatrix=IdentityMatrix();
...
//; and projected ImgGravCenter.
	double Accu_X = 0, Accu_Y = 0, temp_x=0, temp_y=0, 
	...
	double img_mean_x=0, img_mean_y=0, alpha_rad=0, beta_rad = 0;
...
//DEL void CIIPM::Set_InitRotEuler(double PsiGrad, double ThetaGrad, double PhiGrad)
//DEL {
//DEL 	double psi = PsiGrad/180 * pi, phi =PhiGrad/180 * pi,theta = ThetaGrad/180 * pi;
...
//DEL void CIIPM::Set_Params_Camera(double afocuslength, double aCellSizeX, double aCellSizeY, 
//DEL 								int aPixNumX, int aPixNumY, double aPrincipalPointX, 
//DEL 								double aPrincipalPointY){
...
//DEL void CIIPM::Calc_AngleAndAxisFromRotMat(double &aAngleGrad, D3DVECTOR &aDirection, const D3DMATRIX aRotMat){
//DEL 	double aAngle = acos( 0.5 * ( aRotMat(0,0) + aRotMat(1,1) + aRotMat(2,2) - 1 ) );
...
void CIIPM::Calc_Weights(){
	...
	int index = 0;
	vector< double > eiksvec, eiksvectemp; list< double > eikslist;
...
/*void CIIPM::Set_InputData(vector< D3DVECTOR, D3DVECTOR& > aObjPts, const vector< D3DVECTOR, D3DVECTOR& >& aImgPts, double* aNomTrans, double* aNomRot){
...
//DEL  void CIIPM::Set_ProjectionData(int aPixNumX, int aPixNumY, double aFocalLength) {
//DEL  	fCamera.PixNumX = aPixNumX; 	fCamera.PixNumY = aPixNumY; 	fCamera.FocalLength = aFocalLength; }

//DEL void CIIPM::Set_NominalPosition(double *aNomTrans, double *aNomRot){
...
//DEL 	fTransl_Nominal[1] = aNomRot[1];	fTransl_Nominal[2] = aNomRot[2];}
 ...
 void CIIPM::Set_Calibration_Matrix(double aTransCalibration[4][4]){
...
void CIIPM::Calc_Undistortion(){
	...
	double r2=0, r4=0, r6=0, x=0, y=0;	vec(2)=1;	
	if(fPts_Img.size())
	for( int i=0; i<fPts_Img.size(); i++){		
...
void CIIPM::Calc_RegIIPM_OneStep()
{
		...
		//; and saved in the PointPair fWeightedMeans 
		Calc_CrossCorrelation_Matrix(); Calc_Transformation_Matrix(); Transform_Pts_Obj();
		double mean=MeanValue(fDistances,fPts_Corresponding.size());
		Calc_DistanceMatrix();
		Calc_CorrespondingPoints();//; for every projection- ray a 3D- Object- Point is being found
		//Calc_Weights();	
		Calc_WeightedMeans();
...
void CIIPM::Reset_TransInit()
{
	...
	//fTrans_Overall = IdentityMatrix();	fTrans_Camera  = fTrans_Init;
	for( int i=0; i<fPts_Obj.size(); i++ )
		fPts_Obj[i] = TransformVector(fTrans_Init, fPts_Fixed[i]);
