// MtqVFWGrabber.cpp: Implementierung der Klasse MtqVFWGrabber.
//
//////////////////////////////////////////////////////////////////////

#include "stdafx.h"
#include "MtqVFWGrabber.h"
//#include "ipl.h"
#include <assert.h>
//#include "MtqImageIO.h"

//#include "Bitmap.h"
string GrabberTypeString( EGrabberType const aGrabberType )
{
	switch( aGrabberType )
	{
	case kImagraphImascan:
		return "Imagraph Imascan";
		break;

	case kImagingTechnologyITEX:
		return "Imaging Technology ITEX";
		break;

	case kImagenationPXC200:
		return "Imagenation PXC200";
		break;
...
int MtqVFWGrabber::StartLiveVideo( CWnd * apFrameWnd, HWND parent, CView * apView )
{
	if(!fbLive)
	{
...
void MtqVFWGrabber::SaveFrameToBmp()
{
...
void MtqVFWGrabber::SaveFrameToBmp(char* name)
{
	if( !capFileSaveDIB(m_hWndC,name) )
		AfxMessageBox("Fehler in SaveFrameToBmp!"); 
}  

void MtqVFWGrabber::GrabFrame()
{
...
void MtqVFWGrabber::OnFrame( BYTE* data, int width, int height, int format, int bpp )
{
/*const int mirror = 1;
...
/*void MtqVFWGrabber::Show( HDC dc, int x, int y, int w, int h, int from_x, int from_y )
{
if( m_img.width > 0 )
{
uchar buffer[sizeof(BITMAPINFOHEADER) + 1024];
...
int MtqVFWGrabber::StopLiveVideo( HWND aWnd )
{
...
/*	int MtqVFWGrabber::SnapShot( HWND aWindowHandle, MtqGreyImage & aImg, CView * apView )
{
char str[300];
...
bool const MtqVFWGrabber::IsInitialised()
{
	return 0;
}	
//; Check, if the grabber was initialized properly

int MtqVFWGrabber::SelectCamera( int const aCamNr )
{
	return 0;
}
//; Switch to other camera

int MtqVFWGrabber::FitWindow( CView * apView )
{
	//apView->GetParent()->SetWindowPos( NULL, 0, 0, 752, 576, SWP_NOMOVE | SWP_SHOWWINDOW | SWP_NOZORDER );			
	return 1;
} 

/*int MtqVFWGrabber::SnapShot( MtqGreyImage & aImg )
{
return 0;
	}*/

void MtqVFWGrabber::ChooseSource()
{
	// Video source dialog box. 
	if (CapDrvCaps.fHasDlgVideoSource)
...
void MtqVFWGrabber::ChooseFormat()
{
	// Video format dialog box. 
	if (CapDrvCaps.fHasDlgVideoFormat)
...
void MtqVFWGrabber::ChooseDisplay()
{
	// Video display dialog box. 
	if (CapDrvCaps.fHasDlgVideoDisplay)
...
CImage* MtqVFWGrabber::GetStaticImage()
{
